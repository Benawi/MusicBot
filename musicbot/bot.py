...
class MusicBot(discord.Client):
    ...
    async def cmd_play_next(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
            {command_prefix}play_next song_link
            {command_prefix}play_next text to search for
        ...
        if 'entries' in info:
            ...
            reply_text = "Enqueued **%s** songs to be played next."
            ...
            try:
                # add entry after currently playing song
                entry, position = await player.playlist.add_entry_after_current(song_url, channel=channel, author=author)
